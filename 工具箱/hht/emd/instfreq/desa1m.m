...
% Frequency and amplitude values are smoothed.
%
% Calling sequence-
% [W,A]=desa1m(x, dt)
%
% Input-
%	x	- 2-D matrix x(n,m) of data representing the signal
...
% Dan Xiang (JHU)	 	        May 4, 2002 Modified
%				                (added median of 5 points)
...
% forward diff: for y(n) = x(n) - x(n-1), we repeat the first point
% backward diff: for y(n+1) = x(n+1) - x(n), we repeat the last point
...
%----- Force the ending points the same as the adjacent ones
W(2,:) = W(3,:);
W(1,:) = W(2,:);
W(n,:) = W(n-1,:);
W(n+1, :) = W(n, :);
...
for i=3:n-1
    W(i,:)=median(W(i-2:i+2,:));
    A(i,:)=median(A(i-2:i+2,:));
end
    
%----- Energy operator function: e(n) = x(n)*x(n)-x(n-1)*x(n+1)
function e = psi(x)
[r, c]=size(x);
for i=2:r-1
